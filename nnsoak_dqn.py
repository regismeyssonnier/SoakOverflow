	...
	def PlayX10TerrNN_vs_MCTS(self, ind, agent):
		...
		for _ in range(1):
			with torch.no_grad():
				...
				ACTION = agent.select_actions_multi(input_tensor)  # select_action ajoute le batch
		...
		self.reward2 = 0
		if score > 0:
			self.reward = damage_b * damage_b

		if score < 0:
			self.reward2 = damage_r * damage_r
...
def encode_ALL_RL(grid, red, blue, game):
	...
	input_tensor = encode_playersTALL(red_complete, blue_complete, grid, 10, 20, game)
...
def encode_ALL_RL_numpy(grid, red, blue, game):
	...
	input_tensor = encode_playersTALLNumpy(red_complete, blue_complete, grid, 10, 20, game)
...
class DQNAgent:
	...
	@torch.no_grad()
	def select_actions_multi(self, state):
		"""
		state: torch.Tensor (C,H,W) ou np.ndarray (C,H,W)
		return: list of ints, one action per player
		"""
		...
		q_values = self.q_net(state)  # (1, 5, 5)
		actions = []

		for player_q in q_values[0]:  # player_q: (25,)
			if random.random() < self.eps:
				action = random.randint(0, 4)
			else:
				action = player_q.argmax(dim=0).item()
			actions.append(action)

		return actions  # liste de 5 actions
	...
	def optimize(self):
		...
		# Convertir les listes en tensors rapidement
		...
		# Calcul Q(s,a)
		q_values = self.q_net(state_batch)  # (B, NUM_PLAYERS, NUM_ACTIONS) ou (B, NUM_ACTIONS)
		# Forcer q_values à 3D
		if q_values.dim() == 2:
			q_values = q_values.unsqueeze(1)  # (B, 1, NUM_ACTIONS)

		# Forcer action_batch à 2D (B, NUM_PLAYERS)
		# action_batch : s'assurer qu'il est (B, NUM_PLAYERS)
		if action_batch.dim() == 1:  # (B,)
			action_batch = action_batch.unsqueeze(1).expand(-1, 5)  # (B, NUM_PLAYERS)

		q_sa = q_values.gather(2, action_batch.unsqueeze(2)).squeeze(2)  # (B, NUM_PLAYERS)

		with torch.no_grad():
			# Double DQN : choisir action avec q_net, target value avec q_target
			next_q_values = self.q_net(next_state_batch)
			if next_q_values.dim() == 2:
				next_q_values = next_q_values.unsqueeze(1)
			next_actions = next_q_values.argmax(dim=2, keepdim=True)  # (B, NUM_PLAYERS, 1)

			next_q_target = self.q_target(next_state_batch)
			if next_q_target.dim() == 2:
				next_q_target = next_q_target.unsqueeze(1)
			next_q = next_q_target.gather(2, next_actions).squeeze(2)  # (B, NUM_PLAYERS)

			reward_batch = reward_batch * 10.0
			# S'assurer que reward et done ont la bonne dimension
			if reward_batch.dim() == 1:
				reward_batch = reward_batch.unsqueeze(1).expand(-1, q_sa.size(1))
			if done_batch.dim() == 1:
				done_batch = done_batch.unsqueeze(1).expand(-1, q_sa.size(1))

			target = reward_batch + (1.0 - done_batch) * GAMMA * next_q
			target = 0.9 * target + 0.1 * q_sa.detach()

		#print("reward:", reward_batch.mean().item(),
		#"next_q:", next_q.mean().item(),
		#"target:", target.mean().item(),
		#"q_sa:", q_sa.mean().item())


		print(q_sa.shape, target.shape)
		#print(q_sa.shape, target.shape)
		# Huber loss
...
def Train_DQN():
	...
	agent = DQNAgent(state_channels=9, num_actions=25)
	...
	while time.perf_counter() < MAX_EPISODE_T:
		...
		while True:
			...
			agent.replay.push(state, action, reward, next_state, float(dones))
			episode_reward += reward.mean().item()
