	...
	def PlayX_NN10N(self, ind=1):
		...
		with torch.no_grad():
			...
			for idx, pl in enumerate(player):
				...
				# Encoder l'état
				ip = 0
				if ind == 2:ip = len(self.red)
				state_tensor = encode_ALL_RL_numpy(idx+ip, self.grid, self.red, self.blue, self)  # (C,H,W)
		...
		while True:
			...
			if ind == 1:
				...
				for idx, pl in enumerate(game.blue):
					state = encode_ALL_RL(idx+len(game.red), grid, game.red, game.blue, game)
					statess.append(state)

			won = game.PlayX10TerrNN_vs_NN(ind, agent)

			reward = 0
			if ind == 1:
				reward = compute_reward_from_players_red(won)
				#reward = compute_reward_from_players(game, game.red, game.blue, game.reward)
			else:
			...
			if ind == 1:
				...
				for idx, pl in enumerate(game.blue):
					next_state = encode_ALL_RL(idx+len(game.red), grid, game.red, game.blue, game)
					next_statess.append(next_state)
